...
class ZlibEngine : public z_stream {
    public :
		...
		int filetofile( const char *input,
                      const char *output,
                      int level = 6,
					  bool deflation = true);
		...
		int compressInMemory( const unsigned char *source, 
						unsigned int sourceSize, 
						unsigned char *destination, 
						unsigned int maxbuffersize = 0,
						int level = 6 );

		int decompressInMemory( const unsigned char *source, 
						unsigned int sourceSize, 
						unsigned char *destination, 
						unsigned int maxbuffersize );

		uLong compressBound2(unsigned int sourceSize);
//
