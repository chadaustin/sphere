...
, m_Project(project)
{
  ...
  gridsizer = new wxFlexGridSizer(2);
    gridsizer->Add(new wxStaticText(this, -1, "Tilte"), 0, wxALL, 5);
    gridsizer->Add(m_ProjectTitle = new wxTextCtrl(this, -1, ""), 0, wxALL, 5);
  ...
  if (m_Project->GetItemCount(GT_SCRIPTS) > 0) {
    ...
    m_StartScript->Enable(FALSE);
  }

  m_ProjectTitle->SetValue(m_Project->GetGameTitle());

  m_ScreenWidth->SetValue(m_Project->GetScreenWidth());
  m_ScreenHeight->SetValue(m_Project->GetScreenHeight());
}
