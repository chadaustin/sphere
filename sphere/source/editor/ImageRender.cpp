  ...
  for (int iy = redraw_y; iy < redraw_y + redraw_height; iy++)
    for (int ix = redraw_x; ix < redraw_x + redraw_width; ix++)
    {
      ...
      if (color.alpha == 255 || !show_alpha_mask)
      {
        ...
        RECT Rect = { ix * pixel_width, iy * pixel_height, ix * pixel_width + pixel_width, iy * pixel_height + pixel_height };
      ...
      else
      {
        ...
        SetRect(&Rect,
          ix * pixel_width,
          iy * pixel_height,
          ix * pixel_width + pixel_width / 2,
          iy * pixel_height + pixel_height / 2);
        OffsetRect(&Rect, offsetx, offsety);
  ...
  for (int ty = 0; ty <= num_tiles_y; ++ty) {
    for (int tx = 0; tx <= num_tiles_x; ++tx) {
      ...
      for (int iy = 0; iy < dib_height; ++iy) {
        int sy = (iy + (ty * dib_height)) / pixel_height;
        ...
        for (int ix = 0; ix < dib_width; ++ix)
        {
          ...
          {
            int sx = (ix + (tx * dib_width)) / pixel_width;
