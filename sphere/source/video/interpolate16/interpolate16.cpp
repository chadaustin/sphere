...
void Interpolate(word* dest, int pitch)
{
    if (PixelFormat == RGB565)
    {
        ...
        {
        case I_CENTER:
            Interpolate_Center     (dest, pitch);
            break;

        case I_DIRECTSCALE:
            Interpolate_DirectScale(dest, pitch);
            break;

        case I_EAGLE:
            Interpolate_Eagle      (dest, pitch);
            break;

        case I_2XSAI:
            _2xSaIBitmap(16, ScreenBuffer, ScreenWidth, ScreenHeight, dest, pitch);
            break;

        case I_TVMODE:
            Interpolate_TVMode     (dest, pitch, MASK565);
            break;
    ...
    else
    {
        ...
        case I_2XSAI:
            _2xSaIBitmap(15, ScreenBuffer, ScreenWidth, ScreenHeight, dest, pitch);
            break;

        case I_TVMODE:
            Interpolate_TVMode     (dest, pitch, MASK555);
            break;
