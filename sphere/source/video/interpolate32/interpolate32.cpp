...
BOOL CALLBACK ConfigureDialogProc(HWND window, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_INITDIALOG:
        ...
        {
        case I_CENTER:
            CheckDlgButton(window, IDC_CENTER,      BST_CHECKED);
            break;

        case I_DIRECTSCALE:
            CheckDlgButton(window, IDC_DIRECTSCALE, BST_CHECKED);
            break;

        case I_SMOOTH:
            CheckDlgButton(window, IDC_SMOOTH,      BST_CHECKED);
            break;

        case I_TVMODE:
            CheckDlgButton(window, IDC_TVMODE,      BST_CHECKED);
            break;
...
void Interpolate(void* dest, int pitch)
{
    if (BitsPerPixel == 32)
    {
        ...
        {
        case I_CENTER:
            Interpolate_Center     ((BGRA*)dest, pitch / 4);
            break;

        case I_DIRECTSCALE:
            Interpolate_DirectScale((BGRA*)dest, pitch / 4);
            break;

        case I_SMOOTH:
            Interpolate_Smooth     ((BGRA*)dest, pitch / 4);
            break;

        case I_TVMODE:
            Interpolate_TVMode     ((BGRA*)dest, pitch / 4);
            break;
    ...
    else if (BitsPerPixel == 24)
    {
        ...
        {
        case I_CENTER:
            Interpolate_Center     ((BGR*)dest, pitch / 3);
            break;

        case I_DIRECTSCALE:
            Interpolate_DirectScale((BGR*)dest, pitch / 3);
            break;

        case I_SMOOTH:
            Interpolate_Smooth     ((BGR*)dest, pitch / 3);
            break;

        case I_TVMODE:
            Interpolate_TVMode     ((BGR*)dest, pitch / 3);
            break;
